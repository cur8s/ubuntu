...
Profiles enable modules through `UBUNTU_BOOTSTRAP_MODULES`. Phases call the matching lifecycle step for every enabled module.

Keep module scripts narrow. A Tailscale module should not harden SSH, and an SSH module should not manage Kubernetes settings.
