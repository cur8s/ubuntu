...
The floor is intentionally small. A host conforms to it when the following hold.
...
auditd — the floor's attribution contract is role-level (RFC-004); per-person forensics belong to the access layer. Audit rulesets require curation, generate heavy volume on container hosts, and add per-syscall overhead: they fail the inclusion test. A compliance layer may add auditd; the floor does not fight it.
