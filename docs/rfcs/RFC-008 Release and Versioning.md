...
Collection versions are `MAJOR.MINOR.PATCH`, where major.minor mirror the targeted Ubuntu LTS — `24.4.x` for the 24.04 floor — and the patch number counts collection releases. Semver forbids leading zeros, so `24.4` stands for 24.04; LTS releases are always April releases, so the year alone is unambiguous.
...
The collection is distributed from its git repository only; it is not published to a galaxy registry. Consumers install it, and use-case collections depend on it, through git sources.
