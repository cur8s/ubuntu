...
* The collection is `baseline.ubuntu`, distributed from its git repository (RFC-008: Release and Versioning).
* The converge entry point is the collection playbook `baseline.ubuntu.converge`.
* The reboot acceptance gate is the collection playbook `baseline.ubuntu.validate_reboot`.
...
This RFC remains a draft until the first use-case collection (`baseline.k3s`) has consumed the contract and proven it sufficient. Consuming it is expected to surface whatever is missing.

**Scope**

This RFC enumerates the names, paths, and inputs consumers may rely on.

It does not define the meaning of the controls behind them (RFC-003: Floor Contents, RFC-004: Identity and Trust) or playbook implementation.
