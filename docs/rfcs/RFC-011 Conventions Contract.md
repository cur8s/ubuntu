...
* Operator-invoked patching is the collection playbook `cur8s.ubuntu.patch`: a package upgrade that never removes packages and never reboots.
...
The collection playbooks read their inputs from environment variables: `ANSIBLE_PUB_KEY` and `SYSADMIN_PUB_KEY` name the ed25519 public key files, `LOCK_ACCOUNTS` names the comma-separated accounts whose doors `cur8s.ubuntu.lock_accounts` closes (RFC-005: Accounts and Access), `ADOPT_USER` names the pre-baseline account adoption connects through (RFC-007: Adoption), and `ROTATE_ACCOUNT` with `ROTATE_NEW_PUB_KEY` name the baseline account and its replacement public key for `cur8s.ubuntu.rotate_key` (RFC-004: Identity and Trust). The same two key variables also feed the user-data renderer — `scripts/render-cloud-init.sh` inside the installed collection — which writes the cloud-config to the path named by `CLOUD_INIT_FILE`; the script's location and those three inputs are contract surface (RFC-006: Provisioning defers the rendering implementation here).

The runnable examples in `examples/` and the external provider harnesses (the operator's DigitalOcean harness among them) are consumers of this contract — every one resolves the collection by FQCN, connects through the fixed accounts, and supplies inputs through the environment variables above. Future use-case collections that surface missing conventions add them through revisions to this RFC.
