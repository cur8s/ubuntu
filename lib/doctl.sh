...
lab_ensure_local_ssh_key() {
  [ -f "$DO_SSH_KEY_PATH" ] || die "missing SSH private key: $DO_SSH_KEY_PATH; run ./bin/lab-vm init-key"
  [ -f "${DO_SSH_KEY_PATH}.pub" ] || die "missing SSH public key: ${DO_SSH_KEY_PATH}.pub; run ./bin/lab-vm init-key"
}
...
lab_ssh_target() {
  lab_refresh_state >/dev/null || die "no lab Droplet found; run ./bin/lab-vm create"
  load_lab_state
