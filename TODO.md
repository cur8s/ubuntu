...
- [~] Port the archived example playbooks into `examples/<name>/` dirs, each re-asserting the baseline first and runnable against the lab VM (`mise run example:run <name>`; working-tree collection resolved via the `.generated` symlink). Index with themes + techniques in `examples/README.md`. Done: **docker**. Remaining, in porting order: **zot** (binary → systemd unit), **postgres**, **lynis** + **osquery**, **tailscale** (install + join; secret via env var, overlaps future `cur8s.tailscale`).
