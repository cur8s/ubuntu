...
- [ ] Review the host-arch parameterization (lib.sh qemu_platform_env/make_seed_iso, boot/create firmware split) and the CI workflow (.github/workflows/ci.yml) that runs the same lab on amd64 KVM runners — landed 2026-07-05 with the CI feature. Also: vm:status misreported a running VM as stopped once (2026-07-05); find and fix the stage detection.
