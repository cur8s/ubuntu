...
The baseline covers system updates, security hardening, SSH, firewalling, Tailscale connectivity, osquery for interactive inspection, Lynis auditing, and verification checks. It intentionally does not install workload runtimes or applications; those belong in separate repos.
...
- `install/`: simple package installer scripts; baseline scripts are called by `ubuntu-bootstrap`, optional runtime scripts are not.
